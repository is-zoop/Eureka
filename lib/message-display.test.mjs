...
test("hides internal plan-completion tool calls from the conversation", async () => {
  const { getDisplayableAssistantBlocks, splitFinalAssistantBlocks } = await loadSubject();
  const message = assistant([
    { type: "toolCall", toolCallId: "plan-done-1", toolName: "mark_plan_done", input: { taskIndex: 1 } },
    { type: "text", text: "Completed the first task." },
  ]);
  ...
  assert.equal(splitFinalAssistantBlocks(message).answerBlocks[0].text, "Completed the first task.");
});
